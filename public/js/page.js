$(function(){
  ...
  function initUI() {
    ...
    $('#createModalButton').on('click', createPage);
    $('#createModalInput').keypress(function (event) {
      if (event.keyCode === 13) { createPage(); } // enter
    });

    $('#renameModalButton').on('click', renamePage);
    $('#renameModalInput').keypress(function (event) {
      if (event.keyCode === 13) { renamePage(); }
    });

    $('#deleteModalButton').on('click', deletePage);
